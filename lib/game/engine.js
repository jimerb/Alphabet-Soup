...
export function extendPath(board, path, id) {
  ...
  if (path.at(-2) === id) return path.slice(0, -1);
...
export class WordService {
  ...
  chooseBonusWord(board, exclude, random) {
    const words = this.findReachableWords(board, 4).filter(
      (w) => w.word !== exclude,
    );
    return words.length
      ? words[Math.floor(random() * words.length)].word
      : null;
  }
}
...
export function newGame(seed = Date.now()) {
  ...
  ); // A friendly first word, with separate Q/U elsewhere.
  for (let i = 0; i < 4; i++)
...
export function resolve(state, action, service) {
  ...
  for (let c = 0; c < 7; c++) {
    const missing =
      CAPACITIES[c] - s.board.filter((t) => t.column === c).length;
